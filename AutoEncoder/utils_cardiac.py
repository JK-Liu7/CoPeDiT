...
def get_train_loss(x_in, x_rec, intensity_loss, loss_perceptual):
    ...
    x_incomp = rearrange(x_in['x_incomp'], 'b c h w d -> (b d) c h w').contiguous()
    x_missing= rearrange(x_in['x_missing'], 'b c h w d -> (b d) c h w').contiguous()
    rec_incomp = rearrange(x_rec['x_incomp'], 'b c h w d -> (b d) c h w').contiguous()
    rec_missing= rearrange(x_rec['x_missing'], 'b c h w d -> (b d) c h w').contiguous()
    loss2 = loss_perceptual(rec_incomp, x_incomp) + loss_perceptual(rec_missing, x_missing)
    return loss1 / 2, loss2 / 2

def get_adv_loss(x_rec, discriminator, adv_loss):
    x_rec0 = torch.cat((x_rec['x_incomp'], x_rec['x_missing']), dim=-1)
...
def get_discriminator_loss(x_in, x_rec, discriminator, adv_loss):
    x_in0 = torch.cat((x_in['x_incomp'], x_in['x_missing']), dim=-1)
    x_rec0 = torch.cat((x_rec['x_incomp'], x_rec['x_missing']), dim=-1)
