...
def missing_sample(x_in, missing_num, seed):
    ...
    missing_label = get_label(modality, missing_idx)
    return x_incomp, x_missing, missing_label


def get_label(number, missing_idx):
...
def collate_fn_BraTS(batch, missing_num, return_subject_id=False):
    data, idx = zip(*batch)
    ...
    for i, (sample_idx, item) in enumerate(zip(idx, data)):
        ...
        x_incomp, x_missing, missing_label = missing_sample(img, missing_num, seed)
        x_incomp_list.append(x_incomp)
        x_missing_list.append(x_missing)
        missing_label_list.append(missing_label)
        subject_id_list.append(item['subject_id'])
    outputs = (
    ...
    if return_subject_id:
        outputs = outputs + (list(subject_id_list),)
    return outputs


def collate_fn_IXI(batch, missing_num, return_subject_id=False):
    data, idx = zip(*batch)
    x_incomp_list, x_missing_list = [], []
    missing_label_list = []
    subject_id_list = []
...
def get_loader(args, rank, world_size, train_files_combined, test_files_combined, train_transform, test_transform):
    ...
    return_subject_id = getattr(args, "return_subject_id", False)
    inf_collate_fn = partial(collate_fn, missing_num=args.missing_num, return_subject_id=return_subject_id)
