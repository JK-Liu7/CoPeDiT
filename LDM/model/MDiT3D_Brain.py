...
class MDiT3D(nn.Module):
    ...
    def __init__(
        ...
        self.modal_embed = nn.Parameter(torch.zeros(1, num_modalities, hidden_size), requires_grad=False)
    ...
    def initialize_weights(self):
        ...
        modal_embed = get_1d_sincos_temp_embed(self.modal_embed.shape[-1], self.modal_embed.shape[-2])
        self.modal_embed.data.copy_(torch.from_numpy(modal_embed).float().unsqueeze(0))
    ...
    def forward(self, 
        ...
        x: (N, M, C, H, W, D) tensor of volume inputs
        ...
        batches, modalities, channels, high, weight, depths = x.shape
        x = rearrange(x, 'b m c h w d-> (b m) c h w d')
        x = self.x_embedder(x) + self.pos_embed  
        t = self.t_embedder(timesteps, use_fp16=use_fp16)

        timestep_spatial = repeat(t, 'n d -> (n c) d', c=self.modal_embed.shape[1])
        timestep_modal = repeat(t, 'n d -> (n c) d', c=self.pos_embed.shape[1])
        ...
        prompt_embedding_modal = repeat(context, 'n d -> (n c) d', c=self.pos_embed.shape[1])

        for i in range(0, len(self.blocks), 2):
            spatial_block, modal_block = self.blocks[i:i+2]
            # Spatial Block
            c = timestep_spatial
            x = spatial_block(x, c, self.feat_rope)
            x = rearrange(x, '(b m) t d -> (b t) m d', b=batches, m=modalities)
            # Modal Block
            if i == 0:
                x = x + self.modal_embed
            c = timestep_modal + prompt_embedding_modal
            x = modal_block(x, c)
            x = rearrange(x, '(b t) m d -> (b m) t d', b=batches, m=modalities)

        c = timestep_spatial
        x = self.final_layer(x, c)               
        x = self.unpatchify_3D(x)
        x = rearrange(x, '(b m) c h w d -> b m c h w d', b=batches, m=modalities)
        return x
...
def MDiT3D_L_1(**kwargs):
    return MDiT3D(depth=16, hidden_size=1152, patch_size=1, num_heads=12, **kwargs)

def MDiT3D_L_2(**kwargs):
    return MDiT3D(depth=16, hidden_size=1152, patch_size=2, num_heads=12, **kwargs)

def MDiT3D_L_4(**kwargs):
    return MDiT3D(depth=16, hidden_size=1152, patch_size=4, num_heads=12, **kwargs)

def MDiT3D_L_8(**kwargs):
    return MDiT3D(depth=16, hidden_size=1152, patch_size=8, num_heads=12, **kwargs)

def MDiT3D_B_1(**kwargs):
    return MDiT3D(depth=16, hidden_size=768, patch_size=1, num_heads=12, **kwargs)

def MDiT3D_B_2(**kwargs):
    return MDiT3D(depth=16, hidden_size=768, patch_size=2, num_heads=12, **kwargs)

def MDiT3D_B_4(**kwargs):
    return MDiT3D(depth=16, hidden_size=768, patch_size=4, num_heads=12, **kwargs)

def MDiT3D_B_8(**kwargs):
    return MDiT3D(depth=16, hidden_size=768, patch_size=8, num_heads=12, **kwargs)

def MDiT3D_S_1(**kwargs):
    return MDiT3D(depth=12, hidden_size=288, patch_size=1, num_heads=6, **kwargs)
    
def MDiT3D_S_2(**kwargs):
    return MDiT3D(depth=12, hidden_size=288, patch_size=2, num_heads=6, **kwargs)

def MDiT3D_S_4(**kwargs):
    return MDiT3D(depth=12, hidden_size=288, patch_size=4, num_heads=6, **kwargs)

def MDiT3D_S_8(**kwargs):
    return MDiT3D(depth=12, hidden_size=288, patch_size=8, num_heads=6, **kwargs)


MDiT3D_models = {
    'MDiT3D-L/1':  MDiT3D_L_1,   'MDiT3D-L/2':  MDiT3D_L_2,   'MDiT3D-L/4':  MDiT3D_L_4,   'MDiT3D-L/8':  MDiT3D_L_8,
