...
class DiffusionInferer(Inferer):
    ...
    def __call__(
        ...
        """
        noisy_image = self.scheduler.add_noise(original_samples=inputs_m, noise=noise, timesteps=timesteps)
        inputs = torch.cat([inputs_a, noisy_image], dim=1)
        ...
        prediction = diffusion_model(x=inputs, timesteps=timesteps, context=condition)
    ...
    def sample(
        ...
        for t in progress_bar:
            ...
            model_output = diffusion_model(
                x=image, timesteps=torch.full((image.shape[0], ), t).to(input_noise.device), context=conditioning
            )
...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
        ...
        """
        with torch.no_grad():
            latent_a = autoencoder_model.encode_stage_2_inputs(inputs[0]) * self.scale_factor
            latent_m = autoencoder_model.encode_stage_2_inputs(inputs[1]) * self.scale_factor
            latent = torch.cat([latent_a, latent_m], dim=1)
    ...
    def sample(
        ...
        with torch.no_grad():
            latent_a = autoencoder_model.encode_stage_2_inputs(input_a) * self.scale_factor
            input = torch.cat([latent_a, input_noise], dim=1)
        outputs = sample(
            input_noise=input,
            diffusion_model=diffusion_model,
            scheduler=scheduler,
